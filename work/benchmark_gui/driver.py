...
Template variables (all replaced by auto_labeler before execution):
    from benchmark_gui import BenchmarkGui   e.g. "from myproject.ui import MainWindow"
    app = BenchmarkGui()     e.g. "MainWindow(root)"
    benchmark_gui   human screen name
    C:\\GITHUB Repos\\Auto_Labeler\\auto_doc\\work\\benchmark_gui       absolute path where PNG + JSON are written
    import sys
sys.path.insert(0, r"C:\GITHUB Repos\RoboSim_Navigation_Trainer")     optional pre-construct setup (sys.path lines, env vars)

The driver writes into C:\\GITHUB Repos\\Auto_Labeler\\auto_doc\\work\\benchmark_gui:
...
def main():
    out_dir = Path(r"C:\\GITHUB Repos\\Auto_Labeler\\auto_doc\\work\\benchmark_gui")
    ...
    try:
        ...
        from benchmark_gui import BenchmarkGui
        app = BenchmarkGui()
