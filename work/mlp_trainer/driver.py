...
Template variables (all replaced by auto_labeler before execution):
    from train_mlp import TrainerApp   e.g. "from myproject.ui import MainWindow"
    app = TrainerApp()     e.g. "MainWindow(root)"
    mlp_trainer   human screen name
    C:\\GITHUB Repos\\Auto_Labeler\\auto_doc\\work\\mlp_trainer       absolute path where PNG + JSON are written
    import sys
sys.path.insert(0, r"C:\GITHUB Repos\RoboSim_Navigation_Trainer")     optional pre-construct setup (sys.path lines, env vars)

The driver writes into C:\\GITHUB Repos\\Auto_Labeler\\auto_doc\\work\\mlp_trainer:
...
def main():
    out_dir = Path(r"C:\\GITHUB Repos\\Auto_Labeler\\auto_doc\\work\\mlp_trainer")
