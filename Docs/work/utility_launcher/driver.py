...
def main():
    out_dir = Path(r"C:\\GITHUB Repos\\RoboSim_Navigation_Trainer\\Docs3\\work\\utility_launcher")
    out_dir.mkdir(parents=True, exist_ok=True)
    ...
    try:
        ...
        app = UtilityLauncher(); app.root.update_idletasks(); app.root.update()
        ...
        _capture_one(target, out_dir, "utility_launcher", scaling)
        ...
        for peer, slug in _enumerate_peer_toplevels(target):
            sub_name = f"utility_launcher__{slug}"
